  ...
  return (
    <div>
        ...
        <div id={'slider' + id} className='flex whitespace-nowrap scroll-smooth relative overflow-x-scroll gap-4 scrollbar-hide'>
            {Content?.map((item, id) =>
            <div key={id} className='m-2 min-w-[300px] h-[200px] cursor-pointer relative'>
                ...
                <div className='group w-full h-full hover:bg-black/50 absolute left-0 top-0 mx-auto'>
                    ...
                    <p>{like ? <AiFillHeart onClick={saveShow} className='relative top-4 left-4 group-hover:block hidden' /> : <AiOutlineHeart onClick={saveShow} className='relative top-4 left-4 group-hover:block hidden' />}</p>
